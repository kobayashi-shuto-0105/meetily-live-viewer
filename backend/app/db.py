...
class DatabaseManager:
        ...
        try:
            async with self._get_connection() as conn:
                ...
                try:
                    ...
                    await conn.rollback()
                    logger.error(
                        f"Failed to create process for meeting_id {meeting_id}: {str(e)}",
                        exc_info=True,
                    )
                    raise
                ...
                try:
                    ...
                    if result:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to serialize result for meeting_id {meeting_id}: {str(e)}"
                            )
                    ...
                    if metadata:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to serialize metadata for meeting_id {meeting_id}: {str(e)}"
                            )
                    ...
                    await conn.rollback()
                    logger.error(
                        f"Failed to update process for meeting_id {meeting_id}: {str(e)}",
                        exc_info=True,
                    )
                    raise
        ...
        try:
            async with self._get_connection() as conn:
                ...
                try:
                    ...
                    await conn.rollback()
                    logger.error(
                        f"Failed to save transcript for meeting_id {meeting_id}: {str(e)}",
                        exc_info=True,
                    )
                    raise
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if not existing_meeting:
                    ...
                    logger.info(
                        f"Saved meeting {meeting_id} with folder_path: {folder_path}"
                    )
                else:
        ...
        try:
            async with self._get_connection() as conn:
                ...
                try:
                    ...
                    if cursor.rowcount == 0:
                        logger.error(
                            f"Failed to delete meeting {meeting_id} - no rows affected"
                        )
                        await conn.rollback()
                        return False
                    ...
                    await conn.commit()
                    logger.info(
                        f"Successfully deleted meeting {meeting_id} and all associated data"
                    )
                    return True
                    ...
                    await conn.rollback()
                    logger.error(
                        f"Failed to delete meeting {meeting_id}: {str(e)}",
                        exc_info=True,
                    )
                    return False
    ...
    async def get_model_config(self):
        ...
        async with self._get_connection() as conn:
            ...
            row = await cursor.fetchone()
            return (
                dict(zip([col[0] for col in cursor.description], row)) if row else None
            )
        ...
        try:
            async with self._get_connection() as conn:
                ...
                try:
                    ...
                    await conn.rollback()
                    logger.error(
                        f"Failed to save model configuration: {str(e)}", exc_info=True
                    )
                    raise

        except Exception as e:
            logger.error(
                f"Database connection error in save_model_config: {str(e)}",
                exc_info=True,
            )
            raise
    ...
    async def save_api_key(self, api_key: str, provider: str):
        ...
        try:
            async with self._get_connection() as conn:
                ...
                try:
                    ...
                    if existing_config:
                        # Update existing configuration
                        await conn.execute(
                            f"UPDATE settings SET {api_key_name} = ? WHERE id = '1'",
                            (api_key,),
                        )
                    else:
            ...
            logger.error(
                f"Database connection error in save_api_key: {str(e)}", exc_info=True
            )
            raise
    ...
    async def get_api_key(self, provider: str):
        ...
        async with self._get_connection() as conn:
            cursor = await conn.execute(
                f"SELECT {api_key_name} FROM settings WHERE id = '1'"
            )
            row = await cursor.fetchone()
        ...
        try:
            async with self._get_connection() as conn:
                ...
                try:
                    ...
                    await conn.rollback()
                    logger.error(
                        f"Failed to save transcript configuration: {str(e)}",
                        exc_info=True,
                    )
                    raise
            ...
            logger.error(
                f"Database connection error in save_transcript_config: {str(e)}",
                exc_info=True,
            )
            raise
        ...
        try:
            async with self._get_connection() as conn:
                ...
                try:
                    ...
                    cursor = await conn.execute(
                        "SELECT id FROM transcript_settings WHERE id = '1'"
                    )
                    existing_config = await cursor.fetchone()
                    ...
                    if existing_config:
                        # Update existing configuration
                        await conn.execute(
                            f"UPDATE transcript_settings SET {api_key_name} = ? WHERE id = '1'",
                            (api_key,),
                        )
                    else:
                        # Insert new configuration with default values and the API key
            ...
            logger.error(
                f"Database connection error in save_transcript_api_key: {str(e)}",
                exc_info=True,
            )
            raise
    ...
    async def get_transcript_api_key(self, provider: str):
        ...
        async with self._get_connection() as conn:
            cursor = await conn.execute(
                f"SELECT {api_key_name} FROM transcript_settings WHERE id = '1'"
            )
            row = await cursor.fetchone()
            return row[0] if row and row[0] else ""
    ...
    async def delete_api_key(self, provider: str):
        ...
        async with self._get_connection() as conn:
            await conn.execute(
                f"UPDATE settings SET {api_key_name} = NULL WHERE id = '1'"
            )
            await conn.commit()
