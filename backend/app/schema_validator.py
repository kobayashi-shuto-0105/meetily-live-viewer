...
class SchemaValidator:
    ...
    def _validate_table_schema(
        self, cursor, table_name: str, expected_columns: List[Tuple[str, str, str]]
    ):
        ...
        try:
            ...
            if not cursor.fetchone():
                logger.warning(
                    f"Table {table_name} does not exist - will be created by legacy init"
                )
                return
            ...
            if missing_columns:
                logger.warning(
                    f"Schema validation failed for {table_name}: missing columns {[col[0] for col in missing_columns]}"
                )
