...
def process_transcript(
    base_url, transcript_text, provider, model_name, chunk_size, overlap, meeting_id
):
    ...
    logger.info(
        f"Sending POST request to {url} with model '{provider}/{model_name}' and meeting_id '{meeting_id}'..."
    )
    logger.debug(
    ...
    try:
        ...
        )  # 30s timeout for initial request
        ...
        if "process_id" in response_data:
            ...
            logger.info(
                f"Successfully initiated processing. Process ID received: {returned_process_id}"
            )
            # Optional: Verify if returned_process_id matches the meeting_id sent
            if returned_process_id != meeting_id:
                logger.warning(
                    f"Returned process_id '{returned_process_id}' differs from generated meeting_id '{meeting_id}'. Using returned ID for polling."
                )
        ...
        logger.error(
            f"Could not decode JSON response from {url}. Response text: {response.text}"
        )
        return None
...
def poll_summary_status(base_url, meeting_id_for_polling, interval, max_attempts):
    ...
    logger.info(
        f"Polling status endpoint: {url} (every {interval}s) for meeting_id '{meeting_id_for_polling}'"
    )
        ...
        try:
            ...
            if response.status_code == 202:
                ...
                status = status_data.get(
                    "status", "processing"
                ).lower()  # Assume processing if status missing
            ...
            summary_data = status_data.get(
                "data"
            )  # The actual summary is nested in 'data'
            ...
            if status == "completed":
                ...
                if summary_data:
                    ...
                    logger.error(
                        "Status is 'completed' but 'data' field is missing or empty in the response."
                    )
                    return None
            elif (
                ...
                logger.error(
                    f"Error reported by backend: {error_message or 'Unknown error'}"
                )
                return None
            elif status in [
                "processing",
                "pending",
                "started",
            ]:  # Backend might use these
                # Wait before the next poll (already handled by 202 check, but keep for robustness)
                time.sleep(interval)
            else:
                logger.warning(
                    f"Received unknown status '{status}'. Response: {status_data}. Continuing to poll."
                )
                time.sleep(interval)
            ...
            if e.response is not None:
                ...
                if e.response.status_code == 404:
                    logger.error(
                        f"Meeting ID '{meeting_id_for_polling}' not found on server. Ensure processing started correctly."
                    )
            return None
        except json.JSONDecodeError:
            logger.error(
    ...
    logger.error(
        f"Reached maximum polling attempts ({max_attempts}) without completion."
    )
    return None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test the transcript summarization API workflow."
    )
    ...
    parser.add_argument(
        ...
        help=f"Model provider (default: {DEFAULT_MODEL_PROVIDER})",
    )
    parser.add_argument(
        "--model-name",
        default=DEFAULT_MODEL_NAME,
        help=f"Specific model name (default: {DEFAULT_MODEL_NAME})",
    )
    parser.add_argument(
        "--interval",
        type=int,
    ...
    parser.add_argument(
        ...
        help=f"Maximum polling attempts (default: {DEFAULT_MAX_POLL_ATTEMPTS})",
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
    ...
    parser.add_argument(
        ...
        help=f"Overlap size for processing (default: {DEFAULT_OVERLAP})",
    )
