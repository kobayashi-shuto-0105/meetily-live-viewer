...
補足として、このリポジトリには `audio/` と `audio_v2/` が併存しているため、実装開始前に「どちらの経路で `transcript-update` がemitされるか」を確認する。`audio_v2` 経路でも同等のフックが必要なら同時に対応する。
...
既存の `tokio`, `serde`, `serde_json`, `uuid`, `sqlx` はそのまま使う。`Cargo.toml` にすでにある `tokio-tungstenite` は、WebSocket/RESTを `axum` へ一本化する前提で削除する（併存させない）。
...
また、`ExternalWebState` の `.manage(...)` は `run(...)` のspawnより前に登録する。
...
`broadcast::channel(1024)` は slow consumer で `Lagged` が発生し得るため、WS受信側で `Lagged` を検出したら `/api/sessions/:session_id/transcripts` を再取得して再同期する実装を前提にする。
...
pub async fn run(app: AppHandle) -> anyhow::Result<()> {
    ...
    if addr.ip().is_unspecified() && std::env::var("MEETILY_EXT_TOKEN").is_err() {
        anyhow::bail!("MEETILY_EXT_BIND=0.0.0.0 の場合は MEETILY_EXT_TOKEN を必須にする");
    }
...
`CorsLayer::permissive()` はローカル開発限定とし、LAN公開時は許可originを明示的に限定する。
...
WebSocketは、可能なら `Sec-WebSocket-Protocol` を使ってトークンを渡す。初期実装でクエリパラメータを使う場合は、URLクエリをサーバーログへ残さない（伏せる）運用を必須とする。

```txt
new WebSocket("ws://192.168.1.20:38391/ws", ["bearer.xxxxxxxx"])
```
...
で `Authorization: Bearer <token>` を使って認証する。
...
現在 `recording_commands.rs` では `transcript-update` をlistenして `RECORDING_MANAGER.add_transcript_segment(segment)` している。listener登録箇所が複数あるため、先に共通関数へ寄せてからExternal Web UI連携を入れる。([GitHub][4])
...
`TranscriptsRepository::save_transcript(...)` の `Ok(meeting_id)` 分岐内で追加：

```rust
match TranscriptsRepository::save_transcript(...).await {
    Ok(meeting_id) => {
...
を更新する。さらに `source_transcript_id` は以下の順でマッピングする。

```txt
1. 同一meeting内で sequence_id が一致する候補を優先
2. 候補が複数ある場合は audio_start_time / audio_end_time が最も近い行へ紐づけ
3. いずれも一致しない場合は NULL のまま保持（再処理可能にする）
```
...
### 10.4 保存されなかったsessionの扱い

`api_save_transcript` が呼ばれない（録音破棄など）ケースでは `meeting_id IS NULL` の session が残る。MVPでは以下を行う。

```txt
* session作成から24時間経過した meeting_id IS NULL レコードを定期削除
* UIからも「未保存セッションを破棄」操作を提供（任意）
```
...
```txt
1. トランザクション開始（BEGIN IMMEDIATE）
2. 同じ external_segment_id の既存 active revision を is_active = 0 にする
3. version を +1 して新規INSERT
4. コミット
5. 競合時は409を返す（再取得して再編集）
6. WebSocketで TranscriptRevisionCreated をbroadcast
```
...
`anchor_start` / `anchor_end` は **`raw_text` 基準の文字オフセット** とする。`display_text`（edited_text）とは基準を混在させない。
...
UI表示:
  display_textを表示しつつ、anchor計算はraw_text基準で扱う
```
...
VITE_MEETILY_ACCESS_TOKEN=please-set-long-random-token
```
...
const socket = new WebSocket(
  ...
  [`bearer.${import.meta.env.VITE_MEETILY_ACCESS_TOKEN}`]
);
...
`Lagged` や再接続時は、`/api/sessions/:session_id/transcripts` を再取得してクライアント状態を再同期してからWS購読を再開する。
...
MEETILY_EXT_TOKEN=please-set-long-random-token
```
...
* `audio/` と `audio_v2/` のどちらが本番経路か確認
* `transcript-update` listener の登録箇所を共通関数化する前提を確認
* `Cargo.toml` の `tokio-tungstenite` を外し、`axum` へ一本化する方針を確定
...
* `external_transcript_comments`
* `external_transcript_highlights`
* SQLiteの `PRAGMA foreign_keys = ON` が既存DB初期化で有効化されているか確認
